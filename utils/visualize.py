...
def get_runs_and_cfgs(data_path='data/'):
  runs  = []
  cfgs = []
  for run in glob.glob(f'{data_path}/models/*/*/*/*/*'):
    runs.append(run)
...
def get_run_df(run):
  ...
  anomaly = []
  with (open(f"{run}/anomaly_scores.pickle", "rb")) as openfile:
    while True:
      try:
  ...
  return train_df, val_df, anomaly_df, anomaly_score_df
